...
def train():
    # 读取训练测试数据
    data_train = FaceQuality_DataLoader(config.train_data, config.train_txt, 96, 4)
    ...
    # 定义网络结构
    # Net = FaceQualityNet(channels=[16, 32, 64, 96, 128], lda_outc=32)
    Net = FaceQualitySlim(channels=[16, 32, 64, 128, 256])
    Net.cuda()
...
def train2():
    # 读取训练测试数据
    data_train = FaceQuality_augment(config.train_data, config.train_txt, 96, 10)
    ...
    for epoch in range(config.epochs):
        ...
        lr_cur = 0.01
        epoch_loss = 0
        for i, (images, label, numB) in enumerate(train_loader):
            batch += 1
            # 若GPU可用，将图像和标签移往GPU
            images = images.cuda()
            label = label.cuda()
        ...
        # 打印度量
        print("Epoch {}, TrainLoss: {}".format(epoch, epoch_loss.item()))
    torch.save(Net.state_dict(), config.model_save + "/" + "FaceQuality_final.pth")
