...
class Pose_DataLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        path, head, pose, shouder = self.imgs[index]
        ...
        shouder = int(shouder)
        label = np.zeros(3)
        label[0] = self.label_change(head)
        label[1] = self.label_change(pose)
        label[2] = self.label_change(shouder)
        return img, label
