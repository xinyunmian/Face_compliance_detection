...
from load_data import Pose_DataLoader, Pose_collate
...
def train():
    # 读取训练测试数据
    data_train = Pose_DataLoader(config.train_data, config.train_txt)
    train_loader = data.DataLoader(data_train, batch_size=config.batch_size, shuffle=True, num_workers=0, collate_fn=Pose_collate)
