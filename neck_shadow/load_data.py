...
class Data_augment(object):
    def __init__(self, aug, mir, rot, crop):
        self.aug = aug
        self.mir = mir
        self.rot = rot
        self.crop = crop
...
class Shadow_DataLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        process = Data_augment(augment_ornot, mirror_ornot, rotate_ornot, crop_ornot)
        img = process(img)
        ...
        # label process
        shadow = int(shadow)
        label = self.label_change(shadow)
        return img, label
