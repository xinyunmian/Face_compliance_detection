...
def create_regression_label(imgdirs, txtsave):
    label_classfication = open(txtsave, mode="w+")
    for root, dirs, files in os.walk(imgdirs):
        for file in files:
            lab_class = file.split("_")[0]
            root = root.replace('\\', '/')
            splitroot = root.split("/")
            dirname = splitroot[-1]
            imgpath = dirname + "/" + file
            savedata = imgpath + " " + lab_class
            label_classfication.write(savedata)
            label_classfication.write("\n")
    label_classfication.close()
...
def remove_filesjpg(imgdir, dirremove, save):
    for root, dirs, files in os.walk(imgdir):
        for file in files:
            # filesplit = file.split(" - 副本")
            # filep = filesplit[0] + filesplit[1]
            # filelist = list(file)
            # filelist.pop(-5)
            # filep = "".join(filelist)

            remove_name = dirremove + "/" + file
            save_name = save + "/" + file
            img = cv2.imread(remove_name, cv2.IMREAD_COLOR)
            if img is not None:
                shutil.move(remove_name, save_name)
...
if __name__ == "__main__":
    imgpath = "D:/data/imgs/makeup/crop_face"
    txtpath = "D:/data/imgs/makeup/beauty.txt"
    shuffletxt = "D:/data/imgs/makeup/shuffle_beauty.txt"
    # create_classfication_label(imgpath, txtpath)
    create_regression_label(imgpath, txtpath)
    ...
    dir1 = "D:/data/imgs/makeup/crop_face/2"
    dir2 = "D:/data/imgs/makeup/crop_face/2/4"
    dir3 = "D:/data/imgs/makeup/crop_face/2"
    # remove_space(dir1)
