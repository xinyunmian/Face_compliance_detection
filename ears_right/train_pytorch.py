...
from dataset import DataLoader, ears_collate
from mobile_yolo2 import yolo_type
...
train_loader = data.DataLoader(data_train, batch_size=traincfg.batch_size, shuffle=True, num_workers=0, collate_fn=ears_collate)
    ...
    for epoch in range(traincfg.epochs):
        ...
        for i, (images, target) in enumerate(train_loader):
            ...
            if loss.item() < loss_origin:
                torch.save(net.state_dict(), traincfg.model_save + "/" + "ears_better_{}.pth".format(epoch))
                loss_origin = loss.item()

        adjust_learning_rate(epoch)
        if (epoch % 20 == 0 and epoch > 0):
            torch.save(net.state_dict(), traincfg.model_save + "/" + "ears_0528_{}.pth".format(epoch))
