...
class DataLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(img_num):
            ...
            truth, min_w_h = fill_truth_detection(bboxes, self.config.boxes_maxnum, self.config.classes, flip, pleft,
    ...
    def transpose_label(self, img, boxes):
        ...
        label = np.zeros((self.config.boxes_maxnum, 6))
        cc = 0
        for b in boxes:
            x1 = b[0]
            y1 = b[1]
...
if __name__ == "__main__":
    ...
    random.seed(2020)
    np.random.seed(2020)
    datap = "/home/batman/WorkSpace/xym/images/earss.txt"
    dataset = DataLoader(datap, datacfg)
    for i in range(100):
