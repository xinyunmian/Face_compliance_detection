...
train_loader = data.DataLoader(data_train,batch_size=config.batch_size,shuffle=True,num_workers=4)
test_loader = data.DataLoader(data_test,batch_size=config.batch_size,shuffle=False,num_workers=4)
...
def train():
    ...
    for epoch in range(config.epochs):
        ...
        if train_acc > best_acc:
            torch.save(myNet.state_dict(), config.model_save + "/" + "mouth_mask_{}.pth".format(epoch))
            best_acc = train_acc
